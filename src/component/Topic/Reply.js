...
class Reply extends Component {
    ...
    handleStarReply(replyId, replyLoginname, upState){
        ...
        /** todo: 这里有问题，给自己点赞之后，下面的代码还是执行了，虽然请求没发送，但是显示了点赞效果*/
        ...
        if(this.props.isLoaded){
            ...
            if(upState){
                ...
                /** 如果未点赞，则ups中增加userID*/
                newUps = this.state.ups.slice();  /* 注意不要再原state上直接修改，先浅拷贝*/
                newUps.push(this.props.userID);
                this.setState({ ups: newUps });
            }
        }
    }
        ...
        return (
                <li data-flex>
                    ...
                    <div className="main" data-flex-box="1">
                        ...
                        <ReplyBox display={this.state.isShowReplyBox ? 'block' : 'none' }
                                   placeholder={`@${author.loginname}`}
                                   replyId={id}
                                   replyTopic={this.props.replyTopic}
                        />
                    </div>
                </li>
        );
...
Reply.propTypes = {
    userID: React.PropTypes.string,   /* 登录用户id*/
    isLoaded: React.PropTypes.bool,   /* 是否处于登录状态*/
    items: React.PropTypes.object,    /* 每条reply的信息*/
    starReply: React.PropTypes.func   /* 点赞操作的action*/
};
